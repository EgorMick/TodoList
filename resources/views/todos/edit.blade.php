...
@section('content')
    <div class="flex flex-col items-center gap-6">
        <h1 class="text-2xl font-semibold text-teal-700">Редактировать задачу</h1>
        <div class="w-full bg-white rounded-lg shadow-lg p-8">
            <form action="{{ route('todos.update', $todo) }}" method="POST" class="flex flex-col gap-6">
                @csrf
                @method('PUT')
                ...
                <div class="flex flex-col">
                    ...
                    <textarea name="description" rows="4"
                              class="border border-gray-300 rounded px-3 py-2 focus:outline-teal-500 focus:ring-1 focus:ring-teal-300 transition">{{ old('description', $todo->description) }}</textarea>
                </div>

                <div class="flex items-center gap-2">
                    <input id="is_done" type="checkbox" name="is_done" value="1" {{ $todo->is_done ? 'checked' : '' }}
                ...
                <button type="submit" class="self-end bg-teal-500 hover:bg-teal-600 text-white px-6 py-2 rounded-md shadow">Сохранить</button>
            </form>
        </div>
    </div>
@endsection
